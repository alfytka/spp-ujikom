      ...
      <div class="cardxy shadow-sm">
        <div class="card-body">
          ...
          <!-- Vertical Form -->
                ...
                <div class="form-group mb-3">
                  ...
                  <select name="kelas" id="kelas" onchange="document.getElementById('formSearch').submit()" class="form-select form-select-smx roundedx @error('kelas') is-invalid @enderror">
                    ...
                    {{-- <option disabled selected hidden>- Pilih nama kelas -</option> --}}
                      <option value="{{ $kelas->id }}" {{ $kelas->id == $kelasx ? 'selected' : '' }}>{{ $kelas->kelas }}</option>
                    @endforeach
                  </select>
                  @error('kelas')
            ...
            <form class="row g-3 mx-0 mx-md-1 mx-lg-1" action="/datapembayaran/create/siswa" method="POST" id="filtername">
              @csrf

              <input type="hidden" name="kelas" value="{{ $kelasx }}">

              <div class="col-12 col-md-12 col-lg-12 ">
                <div class="form-group mb-3">
                  <label for="name" class="form-label mb-1">Nama Siswa</label>
                  <select name="name" id="name" onchange="document.getElementById('filtername').submit()" class="form-select form-select-smx roundedx @error('name') is-invalid @enderror">
                    ...
                    @foreach ($datasiswa as $siswa)
                      ...
                      <option value="{{ $siswa->id }}" {{ $siswa->id == $getsiswa ? 'selected' : '' }}>{{ $siswa->name }}</option>
                    @endforeach
                  </select>
                  @error('name')
                    <span class="invalid-feedback">{{ $message }}</span>
            ...
            <form class="row g-3 mx-0 mx-md-1 mx-lg-1 mb-3" action="{{ route('datapembayaran.store') }}" method="POST">
              ...
              <input type="hidden" name="siswa_id" class="form-control form-control-smx r" value="{{ $getsiswa }}">
          ...
          <form class="row g-3 mx-0 mx-md-1 mx-lg-1 mb-3" action="#">
            ...
            {{-- <div class="card"> --}}
              <div class="card-body profile-card pt-3 d-flex flex-column align-items-center">
                ...
                <h5 class="fw-semibold name mt-3"></h5>
