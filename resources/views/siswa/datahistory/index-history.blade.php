                    ...
                    @foreach ($riwayat as $pembayaran)
                        <div class="accordion-item mb-2" style="border-radius: 15px; border-top: 1px solid #EBEEF4;">
                            <h6 class="accordion-header" id="headingOne">
                                <div class="accordion-button py-3" style="border-radius: 15px;" type="button"
                                    data-bs-toggle="collapse"
                                    data-bs-target="#{{ $pembayaran->bulan_bayar }}{{ $pembayaran->tahun_bayar }}">
                                    ...
                                    <div class="d-block title-me ms-3">
                                        <h6 class="my-auto fs-12">Pembayaran <span
                                                class="d-none d-md-inline d-lg-inline">SPP</span></h6>
                                        <small class="fw-normal">
                                            <i
                                                class="{{ $pembayaran->status == 'diproses' ? 'bi bi-shuffle text-success' : '' }}{{ $pembayaran->status == 'sukses' ? 'bi bi-check-circle-fill text-blue' : '' }}"></i>
                                            {{ $pembayaran->bulan_bayar }}
                                        </small>
                                    </div>

                                    <div class="d-block float-end accordion-btn">
                                        <h6 class="my-auto fs-12 text-end">
                                            Rp{{ number_format($pembayaran->jumlah_bayar, 0, '.', '.') }}</h6>
                                        <small
                                            class="fw-normal float-end fs-sm">{{ $pembayaran->created_at->diffForHumans() }}</small>
                                    </div>

                                </div>
                            </h6>
                            <div id="{{ $pembayaran->bulan_bayar }}{{ $pembayaran->tahun_bayar }}"
                                ...
                                <div class="accordion-body">
                                    ...
                                    <div class="col-12 mb-2">
                                        <div class="row">
                                            <div
                                                class="col-6 {{ $pembayaran->jenis_pembayaran == 'siswa' ? 'me-1 me-md-0' : '' }} col-md-4">
                                                <small class="title-profile-top">Status</small>
                                                <h6
                                                    class="mb-0 fw-normal fs-10 {{ $pembayaran->status == 'diproses' ? 'text-success' : '' }}{{ $pembayaran->status == 'sukses' ? 'text-blue' : '' }}">
                                                    {{ $pembayaran->status }}<i
                                                        class="px-2 {{ $pembayaran->status == 'diproses' ? 'bi bi-shuffle text-success' : '' }}{{ $pembayaran->status == 'sukses' ? 'bi bi-check-circle-fill text-blue' : '' }}"></i>
                                                </h6>
                                            </div>
                                            <div
                                                class="col-6 {{ $pembayaran->jenis_pembayaran == 'petugas' ? 'col-md-6' : 'col-md-4' }}">
                                                <small class="title-profile-top">Jenis Pembayaran</small>
                                                <h6 class="mb-0 fw-normal fs-10">
                                                    {{ $pembayaran->jenis_pembayaran == 'siswa' ? 'Mandiri' : 'Langsung' }}
                                                    oleh {{ $pembayaran->jenis_pembayaran }}</h6>
                                            </div>
                                            <div class="col-6 col-md-4">
                                                @if ($pembayaran->jenis_pembayaran == 'siswa')
                                                    <small class="title-profile-top">Metode Pembayaran</small>
                                                    <h6 class="mb-0 fw-normal fs-10">{{ $pembayaran->metode_pembayaran }}
                                                    </h6>
                                                @endif
                                            </div>
                                        </div>
                                        @if ($pembayaran->jenis_pembayaran == 'siswa')
                                            <div class="row mt-3">
                                                <div class="col-12">
                                                    <h6 class="fw-normal fst-italic fs-sm"><a href="#buktipembayaran"
                                                            data-bs-toggle="collapse">Lihat bukti pembayaran saya <i
                                                                class="bi bi-chevron-down"></i></a></h6>
                                                    <div class="collapse" id="buktipembayaran">
                                                        <div class="card card-body mb-0">
                                                            <img src="/img/photo-siswa/{{ $pembayaran->buktiPembayaranSiswa[0]->foto_bukti }}"
                                                                class="roundedx mx-auto"
                                    ...
                                    <div class="col-12 mb-2">
                                        <div class="row">
                                            <div class="col-4">
                                                ...
                                                <h6 class="mb-0 fw-normal fs-10">{{ $pembayaran->userSiswa->kelas->kelas }}
                                                </h6>
                                            </div>
                                            <div class="col-5">
                                                <small class="title-profile-top">Tahun SPP</small>
                                                <h6 class="mb-0 fw-normal fs-10">{{ $pembayaran->userSiswa->spp->tahun }}
                                                </h6>
                                            </div>
                                        </div>
                                    </div>
